...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
    <x-ui.card class="border-none shadow-sm">
        <x-ui.card-content class="pt-6">
            ...
            <div class="text-sm text-muted-foreground mt-1">Toplam Mesaj</div>
        </x-ui.card-content>
    </x-ui.card>
    <x-ui.card class="border-none shadow-sm">
        <x-ui.card-content class="pt-6">
            <div class="text-2xl font-semibold text-yellow-600">{{ $stats['unread'] ?? 0 }}</div>
            <div class="text-sm text-muted-foreground mt-1">Okunmamış</div>
        </x-ui.card-content>
    </x-ui.card>
    <x-ui.card class="border-none shadow-sm">
        <x-ui.card-content class="pt-6">
            <div class="text-2xl font-semibold text-blue-600">{{ $stats['internal'] ?? 0 }}</div>
            <div class="text-sm text-muted-foreground mt-1">İç Mesaj</div>
        </x-ui.card-content>
    </x-ui.card>
    <x-ui.card class="border-none shadow-sm">
        <x-ui.card-content class="pt-6">
            <div class="text-2xl font-semibold text-green-600">{{ $stats['guest'] ?? 0 }}</div>
            <div class="text-sm text-muted-foreground mt-1">Misafir Mesajı</div>
        </x-ui.card-content>
    </x-ui.card>
</div>

<div class="flex flex-col h-[calc(100vh-320px)] min-h-[600px]">
    <div class="flex flex-1 gap-4 overflow-hidden">
        <!-- Kullanıcı Listesi -->
        @if(!$chatUser && isset($messages) && $messages->count() > 0)
        <div class="w-80 flex-shrink-0 border rounded-lg bg-white dark:bg-surface-dark overflow-hidden flex flex-col">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="font-semibold text-gray-800 dark:text-text-dark">Mesajlaşma Geçmişi</h3>
            </div>
            <div class="flex-1 overflow-y-auto">
                @php
                    ...
                    foreach($messages as $message) {
                        ...
                        $otherUserId = $senderId == auth()->id() ? $message->to_user_id : $senderId;
                        if ($otherUserId && !$uniqueUsers->contains('id', $otherUserId)) {
                            $otherUser = $senderId == auth()->id() ? $message->toUser : $message->fromUser;
                            if ($otherUser) {
                ...
                @endphp
                @foreach($uniqueUsers as $userItem)
                <a href="{{ route('admin.messages', ['to_user_id' => $userItem->id]) }}" class="flex items-center gap-3 p-3 hover:bg-gray-100 dark:hover:bg-surface-dark/50 transition-colors border-b border-gray-100 dark:border-gray-800">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="text-sm font-medium text-gray-800 dark:text-text-dark truncate">{{ $userItem->name ?? 'Kullanıcı' }}</div>
        ...
        <!-- Chat Window -->
        <div class="flex-1 flex flex-col overflow-hidden rounded-lg bg-white shadow-xl dark:bg-surface-dark">
            <!-- Header Bar -->
            <div class="flex items-center justify-between border-b border-gray-200/50 p-4 dark:border-gray-700/50">
                <div class="flex items-center gap-3 min-w-0 flex-1">
                    @if($chatUser)
                        @php
                            $chatUserAvatar = $chatUser->avatar_url ?? null;
                        @endphp
                        <div class="bg-center bg-no-repeat aspect-square bg-cover rounded-full w-10 h-10 shrink-0 bg-primary-dark/20 flex items-center justify-center overflow-hidden">
                            @if($chatUserAvatar)
                                <img src="{{ asset($chatUserAvatar) }}" alt="User avatar" class="w-full h-full rounded-full object-cover">
                            @else
                                <span class="text-sm font-medium text-primary-dark">{{ strtoupper(substr($chatUser->name ?? 'U', 0, 2)) }}</span>
                            @endif
                        </div>
                        <div class="min-w-0 flex-1">
                            <h1 class="text-base font-bold text-gray-800 dark:text-text-dark truncate">{{ $chatUser->name ?? 'Kullanıcı' }}</h1>
                            <div class="flex items-center gap-1.5">
                                <div class="h-2 w-2 rounded-full bg-green-500"></div>
                                <p class="text-xs font-normal leading-normal text-gray-500 dark:text-gray-400">Çevrimiçi</p>
                            </div>
                        </div>
                    @else
                        <div class="min-w-0 flex-1">
                            <h1 class="text-base font-bold text-gray-800 dark:text-text-dark">Mesajlar</h1>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Mesajlaşma geçmişi olan kullanıcıları seçin</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="flex flex-col gap-4 min-h-full">
                        ...
                        @forelse($messages ?? [] as $message)
                            ...
                            @else
                                ...
                                <div class="flex items-end gap-3 self-start max-w-[80%] md:max-w-[60%]">
                                    @php
                                        $userAvatar = $message->fromUser->avatar_url ?? null;
                                    @endphp
                                    <div class="bg-center bg-no-repeat aspect-square bg-cover rounded-full w-8 h-8 shrink-0 flex items-center justify-center overflow-hidden" style="background-image: url('{{ $userAvatar ? asset($userAvatar) : '' }}'); background-color: {{ $userAvatar ? 'transparent' : 'rgba(0, 146, 202, 0.2)' }};">
                                        @if(!$userAvatar)
                    ...
                    @else
                        <div class="flex flex-col items-center justify-center h-full text-center py-12">
                            ...
                            <p class="text-gray-600 dark:text-gray-400 text-sm mb-1">Bir kullanıcı seçin</p>
                            <p class="text-gray-500 dark:text-gray-500 text-xs">Mesajlaşma geçmişi olan kullanıcıları görmek için sol panelden seçin</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="mt-auto border-t border-gray-200/50 p-3 sm:p-4 dark:border-gray-700/50">
                <form method="POST" action="{{ route('admin.messages.store') }}" id="chat-form">
                    @csrf
